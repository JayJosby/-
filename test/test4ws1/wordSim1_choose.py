...
import pandas as pd
import os
#首先构造一个(当然不止3个)dict：dict dictW2V dictM
...
file = dir + '\\res_Edict_text+voice.txt'
fileW2V = dir + '\\resW2V_Edict_text+voice.txt'
fileM = dir + '\\resM_Edict_text+voice.txt'

newmarkfile = dir + '\\mark_Edict_text+voice.txt'

num = '1'

wordnum = 0

def initDict(wordnum):
    f = open('C:\\Users\Administrator\Desktop\论文\数据\测试数据\wordsim\en\\'+dataset+'.txt', 'r', encoding='utf-8')
    f_Mmodel = open('C:\\Users\Administrator\Desktop\论文\任务记录\词相似度\英文\第一次实验\Edict_text+voice.txt', 'r', encoding='utf-8')
    ...
    #存储匹配ws353规模的dict数据
    isDir('C:\\Users\Administrator\Desktop\论文\任务记录\词相似度\英文\第一次实验\\'+dataset+'\\'+dir)
    f_dict = open('C:\\Users\Administrator\Desktop\论文\任务记录\词相似度\英文\第一次实验\\'+dataset + file, 'w',encoding ='utf-8')
    f_dictW2V = open('C:\\Users\Administrator\Desktop\论文\任务记录\词相似度\英文\第一次实验\\'+dataset + fileW2V, 'w',encoding ='utf-8')
    f_dictM = open('C:\\Users\Administrator\Desktop\论文\任务记录\词相似度\英文\第一次实验\\'+dataset + fileM, 'w',encoding ='utf-8')
    ...
    while line:
        ...
        if w[0] in dict_Mmodel and w[1] in dict_Mmodel and len(dict_Mmodel[w[0]]) == len(dict_Mmodel[w[1]]):
            ...
            r_W2V = round(model.similarity(w[0].replace(' ',''),w[1].replace(' ','')), 2)
...
def getCorrcoef():
    f_dict = open('C:\\Users\Administrator\Desktop\论文\任务记录\词相似度\英文\第一次实验\\' + dataset  + file, 'r',
                  encoding='utf-8')
    f_dictW2V = open('C:\\Users\Administrator\Desktop\论文\任务记录\词相似度\英文\第一次实验\\' + dataset + fileW2V, 'r',
                     encoding='utf-8')
    f_dictM = open('C:\\Users\Administrator\Desktop\论文\任务记录\词相似度\英文\第一次实验\\' + dataset  +  fileM, 'r',
                   encoding='utf-8')
    ...
    f = open('C:\\Users\Administrator\Desktop\论文\任务记录\词相似度\英文\第一次实验\\'+ dataset + newmarkfile, 'w', encoding='utf-8')
